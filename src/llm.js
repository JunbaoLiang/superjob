...
/** 每 100 万 token 的美元价格；未知模型不估价，避免使用错误费率。 */
const PRICING = {
  anthropic: {
...
/**
 * 可注入 clients 的工厂，使路由测试不需要网络或真实 API key。
 * clients.anthropic / clients.openai 均为对应 SDK client。
 */
export function createLlm({ provider = config.llmProvider, model = config.llmModel, env = process.env, clients = {} } = {}) {
  ...
  function validate() {
    if (!PROVIDERS.has(selectedProvider)) {
      throw new Error("LLM_PROVIDER 必须设为 anthropic 或 openai。");
    }
