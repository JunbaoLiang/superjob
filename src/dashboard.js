...
.metricgrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:10px 0}.metric{background:#f6f7f9;border-radius:8px;padding:8px}.metric b{display:block;font-size:18px;color:var(--accent)}
</style></head><body>
<header>
  <h1>Job Copilot</h1>
  ...
  <button class=ghost onclick="toggleMetrics()">本地指标</button>
...
<div class=batchpanel id=metricspanel hidden><button class=x onclick="toggleMetrics()">✕</button><h2>本地指标</h2><p class=meta>仅聚合状态、评分建议、readiness 和来源域名；不显示职位或材料内容。</p><div class=batchresult id=metricsresult>加载中…</div></div>
...
function metricRows(o){return Object.keys(o||{}).sort().map(function(k){return '<div>'+esc(k)+'：'+esc(o[k])+'</div>';}).join("");}
function toggleMetrics(){var p=document.getElementById("metricspanel");p.hidden=!p.hidden;if(!p.hidden){api("/api/metrics").then(function(d){var f=d.funnel||{};document.getElementById("metricsresult").innerHTML='<div class=metricgrid><div class=metric><b>'+esc(d.total)+'</b>总岗位</div><div class=metric><b>'+esc(f.active||0)+'</b>活跃</div><div class=metric><b>'+esc(f.ready||0)+'</b>材料 ready</div></div><b>投递漏斗</b>'+metricRows(f)+'<br><b>建议</b>'+metricRows(d.recommendation)+'<br><b>来源</b>'+metricRows(d.sources);}).catch(function(){document.getElementById("metricsresult").textContent="本地服务连接失败。";});}}
